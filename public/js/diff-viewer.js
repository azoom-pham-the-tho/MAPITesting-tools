...
const DiffViewer = {
  ...
  /**
* Render API diff table (progressive for large datasets)
* @param {HTMLElement} container - Container element
...
*/
  ...
  <table class="api-diff-table">
    ...
    <tbody id="api-diff-tbody"></tbody>
  </table>`;

    const tbody = container.querySelector('#api-diff-tbody');
    const entries = Array.from(requestMap.entries());

    // Progressive rendering for large API tables (>50 rows)
    if (entries.length > 50 && window.ProgressiveRenderer) {
      const renderer = new ProgressiveRenderer({
        chunkSize: 80,
        priorityChunkSize: 25
      });
      renderer.render(entries, tbody, ([key, item]) => {
